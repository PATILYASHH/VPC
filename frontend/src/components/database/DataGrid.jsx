...
function renderCell(value, dataType) {
  ...
  if (dataType?.includes('timestamp')) {
    try {
      return <span className="text-xs">{format(new Date(value), 'MMM d, yyyy HH:mm')}</span>;
    } catch {
      return String(value);
    }
  }
  ...
  if (dataType === 'date') {
    try {
      return <span className="text-xs">{format(new Date(value), 'MMM d, yyyy')}</span>;
    } catch {
      return String(value);
    }
  }

  if (['integer', 'bigint', 'numeric', 'real', 'double precision', 'smallint'].includes(dataType)) {
...
export default function DataGrid({ schema, table, onEditRow }) {
  const [pagination, setPagination] = useState({ pageIndex: 0, pageSize: 50 });
  const [sorting, setSorting] = useState([]);

  const { data, isLoading } = useQuery({
    queryKey: ['table-data', schema, table, pagination, sorting],
    queryFn: () =>
      api
        .get(`/admin/db/table/${table}`, {
          params: {
            schema,
