  ...
  const executeQuery = async (confirm = false) => {
    ...
    try {
      ...
      if (data.requiresConfirmation) {
        if (window.confirm('This is a write operation (INSERT/UPDATE/DELETE/etc). Are you sure you want to execute it?')) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-b">
        <textarea
          className="w-full h-32 rounded-md border border-input bg-transparent px-3 py-2 text-sm font-mono resize-y"
          value={sql}
