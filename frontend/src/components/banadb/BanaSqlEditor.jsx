...
ADD COLUMN new_column TEXT DEFAULT '';`,
  ...
  { label: 'Create index', sql: 'CREATE INDEX idx_table_column ON table_name (column_name);' },
  {
...
const HISTORY_KEY = 'vpc-sql-history';
    ...
    async (confirm = false) => {
      ...
      try {
        ...
        if (data.results) {
          ...
          // Backward compat: wrap single result
          setResults({
        ...
        saveToHistory(project.id, sql.trim());
      } catch (err) {
                      ...
                      {activeResult.rows.map((row, i) => (
                        <tr key={i} className="border-b hover:bg-accent/30 transition-colors">
                          ...
                          {activeResult.fields?.map((f) => (
                            <td
                              key={f.name}
                              className="p-2 max-w-[300px] truncate font-mono"
                            >
                              {row[f.name] === null ? (
                                ...
                                <span
                                  className={
                                    row[f.name] ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'
                                  }
                                >
                                  {String(row[f.name])}
                                </span>
