...
exports.vpcDir = vpcDir;
exports.hasVpcRepo = hasVpcRepo;
exports.hashObject = hashObject;
exports.objectPath = objectPath;
exports.objectExists = objectExists;
exports.writeObject = writeObject;
exports.readObject = readObject;
exports.readObjectRaw = readObjectRaw;
exports.createBlob = createBlob;
exports.readBlob = readBlob;
exports.serializeTreeEntries = serializeTreeEntries;
...
exports.collectReachableObjects = collectReachableObjects;
const crypto = __importStar(require("crypto"));
const zlib = __importStar(require("zlib"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
function vpcDir(root) {
    return path.join(root, '.vpc');
}
function hasVpcRepo(root) {
...
function buildTreeFromFiles(root, files) {
    ...
    for (const f of rootFiles) {
        entries.push({ mode: f.mode, name: f.name, hash: f.hash, type: 'blob' });
    }
...
function collectReachableObjects(root, commitHash, excludeSet = new Set()) {
    const objects = new Set();
    const queue = [commitHash];
    const visited = new Set();
    while (queue.length > 0) {
        const hash = queue.shift();
        if (!hash || visited.has(hash) || excludeSet.has(hash)) {
            continue;
        }
        visited.add(hash);
        objects.add(hash);
        try {
            ...
            for (const parent of commit.parents) {
                if (!visited.has(parent) && !excludeSet.has(parent)) {
                    queue.push(parent);
                }
            }
        }
...
function collectTreeObjects(root, treeHash, objects, excludeSet) {
    if (objects.has(treeHash) || excludeSet.has(treeHash)) {
        return;
    }
    ...
    try {
        const entries = readTree(root, treeHash);
        for (const entry of entries) {
            if (excludeSet.has(entry.hash)) {
                continue;
            }
            if (entry.type === 'tree') {
                collectTreeObjects(root, entry.hash, objects, excludeSet);
            }
            else {
