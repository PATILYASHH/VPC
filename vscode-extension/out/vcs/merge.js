...
// ─── Ancestry ───────────────────────────────────────────────
function findMergeBase(root, a, b) {
    if (a === b)
        return a;
    const ancA = new Set();
    const ancB = new Set();
    ...
    while (qA.length || qB.length) {
        if (qA.length) {
            ...
            if (!ancA.has(h)) {
                ancA.add(h);
                try {
                    for (const p of objects.readCommit(root, h).parents)
                        if (!ancA.has(p))
                            qA.push(p);
                }
                catch { /* end */ }
...
function myersDiff(a, b) {
    ...
    if (MAX === 0)
        return [];
    if (N === 0)
        return b.map(v => ({ type: 'insert', value: v }));
    if (M === 0)
        return a.map(v => ({ type: 'delete', value: v }));
    const vSize = 2 * MAX + 1;
    ...
    return a.map(val => ({ type: 'delete', value: val }))
        .concat(b.map(val => ({ type: 'insert', value: val })));
}
function buildEditScript(trace, a, b, MAX) {
    ...
    for (let d = trace.length - 1; d >= 0 && (x > 0 || y > 0); d--) {
        ...
        if (k === -d || (k !== d && prev && prev[k - 1 + MAX] < prev[k + 1 + MAX]))
            prevK = k + 1;
        else
            prevK = k - 1;
        ...
        if (d > 0) {
            if (x === prevX && y > prevY) {
                y--;
                ops.unshift({ type: 'insert', value: b[y] });
            }
            else if (y === prevY && x > prevX) {
                x--;
                ops.unshift({ type: 'delete', value: a[x] });
            }
        }
    }
    return ops;
...
function threeWayMerge(base, ours, theirs) {
    if (ours === theirs)
        return { merged: ours, hasConflicts: false, conflictCount: 0 };
    if (ours === base)
...
// ─── Full merge ─────────────────────────────────────────────
function mergeTrees(root, ours, theirs) {
    ...
    const baseFiles = baseCommit ? manifestMap(objects.walkTree(root, baseCommit.tree)) : new Map();
    ...
    for (const fp of allPaths) {
        ...
        if (oH === bH) {
            if (tH)
                mergedFiles.push({ path: fp, hash: tH, mode: '100644' });
            continue;
        }
        if (tH === bH) {
            if (oH)
        ...
        if (!oH || !tH) {
            ...
            if (kept)
                mergedFiles.push({ path: fp, hash: kept, mode: '100644' });
            continue;
        }
