...
class DashboardProvider {
    ...
    getInitialState() {
        ...
        const vpshubUrl = config.get('vpshubUrl') || '';
        const vpshubToken = config.get('vpshubToken') || '';
    ...
    async handleSelectRepo(owner, slug) {
        const config = vscode.workspace.getConfiguration('vpcSync');
    ...
    async handleLinkDb(projectId) {
        if (!projectId) {
            return;
        }
        try {
            await this.client.vpshubLinkDb(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo, projectId);
    ...
    async handleUnlinkDb() {
        try {
            await this.client.vpshubUnlinkDb(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo);
    ...
    async handleLinkHosting(hostingId) {
        if (!hostingId) {
            return;
        }
        try {
            await this.client.vpshubLinkHosting(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo, hostingId);
    ...
    async handleUnlinkHosting() {
        try {
            await this.client.vpshubUnlinkHosting(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo);
    ...
    async handleDeploy() {
        try {
            const result = await this.client.vpshubDeployHosting(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo);
    ...
    async handleHostingControl(action) {
        try {
            const result = await this.client.vpshubHostingControl(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo, action);
    ...
    async handleViewLogs() {
        try {
            const data = await this.client.vpshubGetHostingLogs(this.state.vpshubUrl, this.state.vpshubToken, this.state.owner, this.state.repo);
    ...
    renderDashboard() {
        ...
        const branchOptions = s.branches.map(b => `<option value="${this.esc(b)}" ${b === s.branch ? 'selected' : ''}>${this.esc(b)}</option>`).join('');
        // DB project options (exclude already linked)
        const dbOptions = s.availableDbProjects.map(p => `<option value="${this.esc(p.id)}">${this.esc(p.name)} (${this.esc(p.db_name)})</option>`).join('');
        // Hosting project options
        const hostingOptions = s.availableHostingProjects.map(p => `<option value="${this.esc(p.id)}">${this.esc(p.name)} (${p.type === 'static' ? 'Static' : p.type === 'node' ? 'Node.js' : 'Fullstack'})</option>`).join('');
        // Activity
...
DashboardProvider.viewType = 'vpcSync.dashboard';
