...
import { getVpcSession, signOut as authSignOut, VPC_AUTH_PROVIDER_ID } from '../auth/deviceFlow';
...
export class PanelProvider implements vscode.WebviewViewProvider {
  ...
  constructor(client: SyncApiClient, ctx: vscode.ExtensionContext, onConnected: () => void) {
    this.client = client;
    this.ctx = ctx;
  ...
  resolveWebviewView(view: vscode.WebviewView) {
    ...
    vscode.authentication.onDidChangeSessions(e => {
      if (e.provider.id === VPC_AUTH_PROVIDER_ID) { this.render(); }
    });
  }

  private async signIn() {
    try {
      const session = await getVpcSession(true);
      if (!session) { return; }
      this.render();
  ...
  private async fetchReposForSession() {
  ...
  private async signOut() {
    const all = await this.client; void all;
    await authSignOut(this.ctx);
    await vscode.authentication.getSession(VPC_AUTH_PROVIDER_ID, ['repo', 'sync'], { clearSessionPreference: true, createIfNone: false });
  ...
  private async cloneRepo(msg: any) {
    const root = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!root) { vscode.window.showErrorMessage('Open a folder first.'); return; }

    const session = await getVpcSession(false);
    if (!session) { vscode.window.showErrorMessage('Not signed in.'); return; }
  ...
  private render() {
    ...
    getVpcSession(false).then(session => {
      if (!this._view) return;
      const config = vscode.workspace.getConfiguration('vpcSync');
  ...
  private connectedHtml(serverUrl: string, username: string, repository: string, root: string): string {
    const branch = refs.getCurrentBranch(root) || 'main';
    let ahead = 0, behind = 0;
    try { const s = sync.getSyncStatus(root); ahead = s.ahead; behind = s.behind; } catch {}
...
function post(c){vscode.postMessage({command:c})}
  ...
  private repoPickerHtml(serverUrl: string, username: string, displayName: string): string {
    const e = (s: string) => s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/"/g,'&quot;');
