...
import { SyncApiClient, SchemaTable } from '../api/client';

interface TableCompareResult {
  existingTables: string[];
  newTables: { name: string; file: string }[];
...
export class SyncActionsProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'vpcSync.syncActions';
  private _view?: vscode.WebviewView;
  private client: SyncApiClient;
  private onAction: () => void;
  private compareResult: TableCompareResult | null = null;
  ...
  private async loadComparison(): Promise<void> {
    ...
    try {
      ...
      const dbTableNames = new Set(schema.tables.map((t: SchemaTable) => t.name));
      ...
      const prChecksums = new Set<string>();
      ...
      const sqlFiles: { name: string; checksum: string; sql: string }[] = [];
      ...
      const newTables: { name: string; file: string }[] = [];
  ...
  private async commitAndPush(title: string, selectedFiles?: string[]): Promise<void> {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const url = config.get<string>('serverUrl');
    const key = config.get<string>('apiKey');
    const outFolder = config.get<string>('outputFolder') || './migrations';
    ...
    const confirm = await vscode.window.showWarningMessage(
      `Push ${filesToPush.length} migration(s) as PR to VPSHUB?\n\nTitle: "${title}"`,
      { modal: true },
      'Push to VPSHUB'
    );

    if (confirm !== 'Push to VPSHUB') { return; }
    ...
    await vscode.window.withProgress(
      ...
      async (progress) => {
          ...
          try {
            ...
            if (result.pull_request) {
              vscode.window.showInformationMessage(
                `PR #${result.pull_request.pr_number} created: "${result.pull_request.title}"`
              );
            }
          } catch (err: any) {
            failed++;
        ...
        if (failed === 0) {
          vscode.window.showInformationMessage(
            `Successfully pushed ${filesToPush.length} migration(s) as PR(s) to VPSHUB!`
          );
        }
  ...
  private async pushSingleFile(fileName: string, title: string): Promise<void> {
    await this.commitAndPush(title || fileName.replace('.sql', ''), [fileName]);
  }

  private async stageFileByName(fileName: string): Promise<void> {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const outFolder = config.get<string>('outputFolder') || './migrations';
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  ...
  private getHtml(): string {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const isConnected = !!(config.get<string>('serverUrl') && config.get<string>('apiKey'));
  ...
  private wrapHtml(body: string, connected: boolean): string {
    return `<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
