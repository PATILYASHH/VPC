...
export class ChangesProvider implements vscode.WebviewViewProvider {
  private _view?: vscode.WebviewView;
  private client: SyncApiClient;
  private context: vscode.ExtensionContext;

  constructor(client: SyncApiClient, context: vscode.ExtensionContext) {
    this.client = client;
  ...
  resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      if (msg.command === 'push') { vscode.commands.executeCommand('vpcSync.push'); }
      else if (msg.command === 'pull') { vscode.commands.executeCommand('vpcSync.pull'); }
      else if (msg.command === 'refresh') { vscode.commands.executeCommand('vpcSync.refresh'); }
    });
  }
...
<script>const vscode = acquireVsCodeApi();</script>
  ...
  private getHtml(branch: string, ahead: number, behind: number, commits: number, headShort: string): string {
