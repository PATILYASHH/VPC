...
export class RepoChangesProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'vpcSync.repoChanges';
  ...
  private async loadState(): Promise<void> {
    ...
    const baseUrl = config.get<string>('vpshubUrl');
    ...
    try {
      ...
      const remoteFiles = new Map<string, { hash: string; size: number }>();
      ...
      changes.sort((a, b) => {
        if (a.isMigration && !b.isMigration) return -1;
        if (!a.isMigration && b.isMigration) return 1;
      ...
      // Load saved SHA
      const savedSha = vscode.workspace.getConfiguration('vpcSync').get<string>('_lastSyncSha') || '';
  ...
  private scanLocalFiles(dir: string, prefix: string, result: Map<string, string>): void {
    try {
      const entries = fs.readdirSync(dir, { withFileTypes: true });
      for (const entry of entries) {
        // Skip common non-project dirs
        if (['node_modules', '.git', '.vscode', 'dist', 'out', '__pycache__', '.next'].includes(entry.name)) continue;
  ...
  private async pullFromVpshub(): Promise<void> {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const baseUrl = config.get<string>('vpshubUrl')!;
    const token = config.get<string>('vpshubToken')!;
    const owner = config.get<string>('vpshubOwner')!;
    ...
    const confirm = await vscode.window.showWarningMessage(
      `Pull ${filesToPull.length} file(s) from VPSHUB? This will overwrite local files.`,
      { modal: true },
      'Pull Files'
    );
    ...
    await vscode.window.withProgress(
      { location: vscode.ProgressLocation.Notification, title: 'VPSHUB Pull' },
      async (progress) => {
        let pulled = 0;
        ...
        if (newMigs.length > 0 && this.state.dbConnected) {
          const autoRun = config.get<boolean>('autoRunMigrations');
          if (autoRun) {
            await this.runNewMigrations();
          } else {
            const runNow = await vscode.window.showInformationMessage(
              `${newMigs.length} new migration(s) detected. Run them now?`,
  ...
  private async pullSingleFile(filePath: string): Promise<void> {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const baseUrl = config.get<string>('vpshubUrl')!;
    const token = config.get<string>('vpshubToken')!;
    const owner = config.get<string>('vpshubOwner')!;
  ...
  private async runNewMigrations(): Promise<void> {
    const config = vscode.workspace.getConfiguration('vpcSync');
    const dbHost = config.get<string>('dbHost') || 'localhost';
    const dbPort = config.get<number>('dbPort') || 5432;
    const dbName = config.get<string>('dbName');
  ...
  private fileItemHtml(file: FileChange, badge: string, color: string): string {
